...
import br.com.wti.school.endpoint.v1.assignment.AssignmentEndpointTest;
import br.com.wti.school.endpoint.v1.choice.ChoiceEndpointTest;
...
public class QuestionAssignmentEndpointTest {
	...
	public static QuestionAssignment mockQuestionAssignment() {
		return QuestionAssignment.Builder.newQuestionAssignment().assignment(AssignmentEndpointTest.mockAssignment())
				.professor(ProfessorEndpointTest.mockProfessor()).question(QuestionEndpointTest.mockQuestion())
				.grade(50).id(1L).build();
	}
	...
	public void setup() {
		...
		BDDMockito.when(questionRepository.listQuestionsByCourseNotAssociatedWithAnAssignment(course.getId(),
				assignment.getId())).thenReturn(Collections.singletonList(question));
		...
		BDDMockito.when(questionAssignmentRepository.findOne(questionAssignment.getId()))
	...
	public void listValidQuestionsForAnAssignmentWhenTokenIsWrongShouldReturn403() throws Exception {
		ResponseEntity<String> exchange = testRestTemplate.exchange(
				"/v1/professor/course/assignment/questionassignment/{courseId}/{assignmentId}", GET, wrongHeader,
				String.class, course.getId(), assignment.getId());
	...
	public void listValidQuestionsForAnAssignmentWhenCourseDoesNotExistShouldReturnEmptyList() throws Exception {
		ResponseEntity<List<Question>> exchange = testRestTemplate.exchange(
				"/v1/professor/course/assignment/questionassignment/-1/1", GET, professorHeader,
	...
	@Test
	public void listValidQuestionsForAnAssignmentWhenAssignmentAndCourseExistAndHasValidQuestionsShouldReturnListWithOneElement()
			throws Exception {
		ResponseEntity<List<Question>> exchange = testRestTemplate.exchange(
				"/v1/professor/course/assignment/questionassignment/1/1", GET, professorHeader,
				new ParameterizedTypeReference<List<Question>>() {
	...
	public void deleteQuestionAssignmentWhenIdExistsShouldReturn200() throws Exception {
		...
		ResponseEntity<String> exchange = testRestTemplate.exchange(
				"/v1/professor/course/assignment/questionassignment/{id}", DELETE, professorHeader, String.class, id);
